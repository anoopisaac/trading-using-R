...
for(i in 1:nrow(macd_tech_data)){
  ...
  if(isPurchaseOn ){
    ...
    cat(buyDate,buyingPrice,currPrice,ratio,i,'\n')
    ...
    if(sellFlag){
      cat('sell================>',buyDate,buyingPrice,currPrice,ratio,i,'\n')
      isPurchaseOn=FALSE
  ...
  if(!isPurchaseOn && isRising(3,macd_tech_data,i,'history')){
    # buyingPrice=as.numeric(macd_tech_data[i,'close'])
    # print(as.numeric(as.character(macd_tech_data[i,]$close)))
